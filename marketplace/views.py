...
def child_category_list(request, pk):
    ...
    if category.get_level() < 2 and category.get_children().count() >= 2:
        ...
        return render(
            request,
            "marketplace/child_category_list.html",
...
def category_products(request, pk):
    ...
    products_qs = Product.objects.filter(
        category_id__in=ids, on_the_main=True
    ).order_by("-id")
    ...
    return render(
        request,
        "marketplace/category_products.html",
        {
            "category": category,
...
def product_detail(request, pk):
    ...
    )  # связанные атрибуты (связь через ProductAttribute)
    same_products = Product.objects.filter(category=product.category).exclude(
        id=product.id
    )[:8]
...
def product_detail_api(request, pk):
    product = get_object_or_404(
        Product.objects.prefetch_related("images", "attributes"), pk=pk
    )
