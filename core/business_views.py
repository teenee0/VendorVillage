...
from marketplace.serializers import CategorySerializer, ProductSerializer, ProductDetailSerializer, ProductListSerializer, ProductVariantSerializer, ProductImageSerializer
...
def product_images_api(request, business_slug, product_id):
    ...
    if request.method == 'GET':
        images = ProductImage.objects.filter(product=product).order_by('-is_main', 'display_order')
...
def get_product_attributes(request, business_slug, product_id):
    try:
        product = get_object_or_404(Product, id=product_id, business__slug=business_slug)
        ...
        for cat_attr in category_attrs:
            attr_data = {
                ...
                ] if cat_attr.attribute.has_predefined_values else []
            }
            attributes.append(attr_data)
        
        # 2. Получаем существующие варианты товара с их атрибутами
        existing_variants = []
        for variant in product.variants.all().prefetch_related('attributes__category_attribute__attribute', 
            ...
            for attr in variant.attributes.all():
                attr_data = {
                    ...
                    'predefined_value_id': attr.predefined_value.id if attr.predefined_value else None,
                    'predefined_value': attr.predefined_value.value if attr.predefined_value else None,
